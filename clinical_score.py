...
class ClinicalScore(BaseEstimator, ClassifierMixin):
    ...
    def get_CHAP_Score(self, X):
        ...
        antiplatelate = (1 if X['Antipiatelet agent']==1 else 0) * 0.8786
    ...
    def get_ACCP_Score(self, X):
        ...
        antiplatelate = 1 if X['Antipiatelet agent']==1 else 0
        
        nseid = 1 if X['Non-steroidal anti-Inflammatory (NSAIDs)']==1 else 0
    ...
    def get_RIETE_Score(self, X):
        ...
        if (X['Pulmonary embolism only']==1) | (X['Deep vein thrombosis and pulmonary embolism']==1):
